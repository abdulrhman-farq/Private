...
// تحميل كسول لمكتبة Supabase: تُستورد ديناميكيًا عند أول استخدام فتُقسَّم في حزمة
// منفصلة ولا تثقل التحميل الأولي (أسرع أول رسم). يُنشأ العميل مرة واحدة ويُخزَّن.
let _clientPromise
function getClient() {
  if (!_clientPromise) {
...
export async function addCustomMessage(key, text, times, days, target, mode, atHour) {
  try { const supa = await getClient(); const { error } = await supa.rpc('add_custom_message', { p_key: key, p_text: text, p_times: times, p_days: days, p_target: target, p_mode: mode || 'spread', p_at_hour: (atHour === null || atHour === undefined) ? null : atHour }); return !error } catch (e) { return false }
}
export async function listCustomMessages(key) {
  try { const supa = await getClient(); const { data, error } = await supa.rpc('list_custom_messages', { p_key: key }); return error ? [] : (data || []) } catch (e) { return [] }
}
export async function deleteCustomMessage(key, id) {
  try { const supa = await getClient(); const { error } = await supa.rpc('delete_custom_message', { p_key: key, p_id: id }); return !error } catch (e) { return false }
}
// إرسال فوري (الآن) عبر دالة الخادم.
export async function sendNow(key, text, target) {
  try { const supa = await getClient(); const { error } = await supa.functions.invoke('send-now', { body: { key, text, target } }); return !error } catch (e) { return false }
}
// نبضة شوق (دفعة قلوب) للطرف الآخر.
export async function sendNudge(key, target, style) {
  try { const supa = await getClient(); const { error } = await supa.functions.invoke('nudge', { body: { key, target, style: style || 'hearts' } }); return !error } catch (e) { return false }
}
